...
object Main {
    ...
    def job() = {
        ...
        val cipherTextBlock = CipherDataBlock.create(cipherText, alphabetNoJ)
        val broken = FourSquareCipherBreaker.break(cipherTextBlock, 50, 1500)

        println(broken.textData.take(100))
        println(
            broken.key
                .map(x => x.values.mkString)
                .mkString("\n")
        )
    }
