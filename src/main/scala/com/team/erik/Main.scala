...
object Main {
    ...
    def job() = {
        ...
        val cipherData = CipherDataBlock.formatAndCreate(ciphertext.reverse)._1

        val broken = VigenereCipherBreaker.break(cipherData)
        println(broken.textData)

        broken.notes = "Reversed Vigenere"
        saveText(broken.textData, ".\\submissions\\Challenge8A.xml")
    }
