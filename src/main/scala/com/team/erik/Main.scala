...
object Main {
    ...
    def job() = {
        ...
        val yProcessed = y.map(_.toString.toInt - 1)

        val combined = xProcessed.zip(yProcessed).flatMap { case (a, b) => List(a, b) }
        println(combined.mkString)

        val data = new CipherDataBlock(combined, PosIntAlphabet)
        val broken = PolybiusCipherBreaker.break(data, 'Z')
        println(broken.textData)

        saveText(broken.toXml, ".\\submissions\\Challenge7B.xml")
    }
