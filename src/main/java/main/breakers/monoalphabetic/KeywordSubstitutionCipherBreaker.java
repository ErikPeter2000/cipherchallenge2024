    ...
    public static CipherBreakerOutput<byte[]> wordlistBruteforce(byte[] cipherText) {
        ...
        for (int i = 0; i < Constants.wordlist.length; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                byte[] text;
                if (j == 0) {
                    text = KeywordSubstitutionCipher.decipher(cipherText, keyword, KeywordSubstitutionCipher.KeyFiller.NORMAL, false);
                } else if (j == 1) {
                    text = KeywordSubstitutionCipher.decipher(cipherText, keyword, KeywordSubstitutionCipher.KeyFiller.LAST_LETTER, false);
                } else {
                    text = KeywordSubstitutionCipher.decipher(cipherText, keyword, KeywordSubstitutionCipher.KeyFiller.ALPHABETICALLY_LAST_LETTER, false);
                }
